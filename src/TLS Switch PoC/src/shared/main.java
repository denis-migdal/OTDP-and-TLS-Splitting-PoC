...
public class main {
	...
	private static void usage() {
		...
		System.out.println(	"\ttlsswitch server [--server-port port]");
		System.out.println(	"\ttlsswitch proxy [--proxy-port port] [--initial-state StateT]");
		System.out.println(	"\ttlsswitch trusted [--proxy-ip ip] [--proxy-port port] [--server-ip ip] [--server-port port] [--switch-mode naive]");
		System.out.println(	"\tjava -javaagent:/poc/bin/agent.jar -jar /poc/bin/tlsswitch.jar trusted [--proxy-ip ip] [--proxy-port port] [--server-ip ip] [--server-port port] [--switch-mode choose|normal]");
	...
	public static void main(String[] args) throws KeyManagementException, NoSuchAlgorithmException, CertificateException, FileNotFoundException, KeyStoreException, IOException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, InstantiationException, NoSuchMethodException, InvocationTargetException, ClassNotFoundException, IllegalBlockSizeException, BadPaddingException, InterruptedException, NotFoundException, CannotCompileException {
		...
		for(int i = 1 ; i < args.length ; i +=2) {
			...
			case "--switch-mode":
				...
				break;
			default:
				System.err.println("Option " + args[i] + " not implemented !");
				bad_usage();
			}
